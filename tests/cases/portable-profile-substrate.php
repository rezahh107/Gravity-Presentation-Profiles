...
gpp_assert_same(
    'PACKAGE_VALID',
    VisualProfilePackage::validationReport( $package )['structural_status'],
    'Package validity must be structural and explicit.'
);
gpp_assert_same(
    'NOT_PROVEN',
    VisualProfilePackage::validationReport( $package )['target_runtime_evidence'],
...
gpp_assert_same(
    array( 'PROVEN', 'UNBOUND', 'NOT_PROVEN', 'NOT_APPLICABLE' ),
    EnvironmentBindingSet::bindingStates(),
    'Binding-state vocabulary must be exact.'
);
gpp_assert_same(
    'BINDING_VALID',
    EnvironmentBindingSet::validationReport( $binding_a )['structural_status'],
    'Binding validity must be structural and explicit.'
);
gpp_assert_true(
    in_array( 'student.full_name', EnvironmentBindingSet::validationReport( $binding_a )['proven_binding_sources'], true ),
    'A PROVEN source must appear only when source evidence is present.'
);
gpp_assert_true(
    in_array( 'student.full_name:availability', EnvironmentBindingSet::validationReport( $binding_a )['proven_runtime_claims'], true ),
    'Runtime claim proof must be separately evidenced.'
);
gpp_assert_true(
    ! in_array( 'school.name:editability', EnvironmentBindingSet::validationReport( $binding_a )['proven_runtime_claims'], true ),
    'Structural validity must not promote NOT_PROVEN editability.'
);

$slot_keys = array_map(
    static function ( $slot ) {
        return $slot['semantic_slot_key'];
    },
...
wu09_expect_violation(
    static function () use ( $visual_resolver ) {
        $visual_resolver->resolve( 'gravity_flow.inbox', array( 'form_id' => 999 ) );
    },
    'Visual resolution must reject environment context and overrides.'
);

$context_a = array(
    'installation_id' => 'fixture-installation',
    'form_id' => 'fixture-form-a',
    'entry_id' => 'fixture-entry-a',
    'surface' => 'gravity_flow.inbox',
);
$context_b = array(
    'installation_id' => 'fixture-installation',
    'form_id' => 'fixture-form-b',
    'entry_id' => 'fixture-entry-b',
    'surface' => 'gravity_flow.inbox',
);
...
gpp_assert_same(
    VisualProfilePackage::contentHash( $package ),
    VisualProfilePackage::contentHash( $package_reordered ),
    'Canonical visual hash must ignore associative object-key order.'
);
...
gpp_assert_true(
    VisualProfilePackage::contentHash( $package ) !== VisualProfilePackage::contentHash( $package_array_reordered ),
    'Canonical visual hash must preserve ordered semantic arrays.'
);

$binding_reordered = wu09_reverse_object_keys( $binding_a );
gpp_assert_same(
    EnvironmentBindingSet::contentHash( $binding_a ),
    EnvironmentBindingSet::contentHash( $binding_reordered ),
    'Canonical binding hash must ignore associative object-key order.'
);

$binding_changed = $binding_a;
$binding_changed['context']['form_source_ref']['form_id'] = 'fixture-form-a-2';
gpp_assert_true(
    EnvironmentBindingSet::contentHash( $binding_a ) !== EnvironmentBindingSet::contentHash( $binding_changed ),
    'Canonical binding hash must change for semantic content changes.'
);

$coerced = $package;
$coerced['design_tokens']['spacing_px']['unit'] = '8';
wu09_expect_violation(
    static function () use ( $coerced ) {
        VisualProfilePackage::validate( $coerced );
    },
    'Silent semantic coercion must be rejected.'
);
...
wu09_expect_violation(
    static function () use ( $bad ) {
        VisualProfilePackage::validate( $bad );
    },
    'Executable PHP payload must be rejected.'
);
...
wu09_expect_violation(
    ...
    'Raw DOM selector passthrough must be rejected.'
);
...
wu09_expect_violation(
    ...
    'Raw hook passthrough must be rejected.'
);
...
wu09_expect_violation(
    ...
    'Concrete form identity must be rejected from profile_id.'
);
...
wu09_expect_violation(
    ...
    'Reserved Extension Seam must remain inert and reject targeting.'
);


$bad = $binding_a;
$bad['context']['form_id'] = 'raw-form-id';
wu09_expect_violation( static function () use ( $bad ) { EnvironmentBindingSet::validate( $bad ); }, 'Raw context identifiers outside typed source references must be rejected.' );
$bad = $binding_a;
$bad['context']['form_source_ref']['type'] = 'arbitrary.form';
wu09_expect_violation( static function () use ( $bad ) { EnvironmentBindingSet::validate( $bad ); }, 'Unknown context source-reference types must be rejected.' );
$bad = $binding_a;
...
wu09_expect_violation(
    static function () use ( $bad ) {
        EnvironmentBindingSet::validate( $bad );
    },
    'Unknown binding state must be rejected.'
);
...
wu09_expect_violation(
    ...
    'PROVEN binding without evidence must be rejected.'
);
...
wu09_expect_violation(
    ...
    'UNBOUND binding must reject guessed source identifiers.'
);
...
wu09_expect_violation(
    ...
    'Arbitrary host API source types must be rejected.'
);
...
wu09_expect_violation(
    ...
    'Binding raw selectors must be rejected.'
);
...
wu09_expect_violation(
    ...
    'PROVEN runtime evidence without provenance must be rejected.'
);

gpp_assert_same(
    CanonicalJson::hash( array( 'b' => 2, 'a' => 1 ) ),
    CanonicalJson::hash( array( 'a' => 1, 'b' => 2 ) ),
    'Canonical JSON must sort object keys.'
);
gpp_assert_true(
    CanonicalJson::hash( array( 1, 2 ) ) !== CanonicalJson::hash( array( 2, 1 ) ),
