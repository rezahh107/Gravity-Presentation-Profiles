...
gpp_assert_true( 1 === preg_match( '/\.gravityflow-timeline \.gravityflow-note-body-wrap > \.gravityflow-note-body\s*\{[^}]*display:\s*grid;[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\)\s+52px\s+minmax\(156px,\s*196px\);/s', $timeline ), 'Desktop Timeline must use the authentic outer native body as the three-zone normal-flow grid owner.' );
...
gpp_assert_true( false !== strpos( $timeline, '.gravityflow-note-body-wrap > .gravityflow-note-body > .gravityflow-note-body' ) && false !== strpos( $timeline, 'background: #f5f8ff' ), 'Authentic nested event text body must become the dominant soft content card.' );
gpp_assert_true( false !== strpos( $timeline, '.gravityflow-note-header' ) && false !== strpos( $timeline, '.gravityflow-note-title' ) && false !== strpos( $timeline, '.gravityflow-note-meta' ) && false !== strpos( $timeline, 'background: #f8fafc' ), 'Authentic native header must become the compact actor/date metadata card.' );
