...
async function test(id, name, fn) {
  ...
  catch (e) { record(id, name, 'FAIL', String(e?.stack || e)); }
}
...
try {
  ...
  await test('WU21-BROWSER-001', 'native AG Grid Inbox renders synthetic tasks', async () => {
  ...
  await test('WU21-BROWSER-002', 'native quick search uses Gravity Flow grid control', async () => {
  ...
  await test('WU21-BROWSER-003', 'native AG Grid sorting and pagination execute', async () => {
  ...
  await test('WU21-BROWSER-004', 'native Entry Details navigation is preserved', async () => {
    await page.goto(inboxUrl, { waitUntil: 'networkidle' });
  ...
  await test('WU21-BROWSER-005', 'native polling refresh adds and removes a synthetic task', async () => {
    await page.goto(inboxUrl, { waitUntil: 'networkidle' });
  ...
  await test('WU21-BROWSER-006', 'reload/re-render retains exactly one native Inbox grid', async () => {
    await page.goto(inboxUrl, { waitUntil: 'networkidle' });
    await page.waitForSelector('[data-js="gflow-inbox"] .ag-root-wrapper', { timeout: 30000 });
