...
foreach ($cases as $key => $case) {
  ...
  $sources = array();
  foreach ($sentinels as $slot => $value) {
    $sources[] = array('slot' => $slot, 'field_id' => (string)$case['fields'][$slot], 'sentinel' => $value, 'expected_hidden' => 'student.home_phone' === $slot);
  }
...
echo wp_json_encode(array(
  ...
  'gravity_forms_version' => defined('GFForms::VERSION') ? GFForms::VERSION : (class_exists('GFForms') ? GFForms::$version : null),
  'gravity_flow_version' => defined('GRAVITY_FLOW_VERSION') ? GRAVITY_FLOW_VERSION : (function_exists('gravity_flow') ? gravity_flow()->get_version() : null),
...
$out = array(
  ...
  'assignee_status' => ($step && method_exists($step, 'get_assignee_status')) ? $step->get_assignee_status() : null,
...
function analyzeHtml(html, item, metadata = {}) {
  const known = item.sources.filter(source => !source.expected_hidden);
  const hidden = item.sources.find(source => source.expected_hidden);
  ...
  const hiddenAnywhere = Boolean(hidden && html.includes(hidden.sentinel));
  const hiddenPlaceholder = Boolean(doc.querySelector('[data-gpp-slot="student.home_phone"]'));
  const subset = gppRendered.every(id => nativeVisible.includes(id));
  return {
    ...metadata,
...
async function capturePage(page, url, item, metadata) {
  ...
  return analyzeHtml(html, item, { ...metadata, http_status: status, final_url: page.url() });
}
...
async function captureOrdinaryPost(page, item) {
  ...
  return analyzeHtml(payload.html, item, {
    state: 'ordinary_post',
    request_method: 'POST',
...
try {
  ...
  const assigneeBefore = hostState(fixture.cases.assignee, 'bootstrap_admin');
  states.push(await capturePage(page, adminEntryUrl(fixture.cases.assignee), fixture.cases.assignee, {
    state: 'current_assignee_active_step_get', request_method: 'GET', host_context: assigneeBefore,
  }));

  const postBefore = hostState(fixture.cases.assignee, 'bootstrap_admin');
  const postCapture = await captureOrdinaryPost(page, fixture.cases.assignee);
  postCapture.host_context_before = postBefore;
  ...
  const viewerCapture = await capturePage(page, frontendEntryUrl(fixture.cases.viewer), fixture.cases.viewer, {
    state: 'authorized_non_assignee_get', request_method: 'GET', host_context: viewerHost,
  });
  ...
  if (await approve.count() === 1) {
    ...
    const completeAfter = hostState(completeItem, 'bootstrap_admin');
    states.push(analyzeHtml(await page.content(), completeItem, {
      state: 'complete_approved_post_response',
      request_method: 'POST',
      authentic_native_approve_control: true,
      host_context_before: completeBefore,
      host_context_after: completeAfter,
      http_status: 200,
      final_url: page.url(),
    }));
...
for (const state of states) {
  ...
  state.hard_gates = {
    ...
    hidden_not_native_visible: hidden ? hidden.native_visible === false : false,
    hidden_not_gpp_visible: hidden ? hidden.gpp_visible === false : false,
    hidden_sentinel_not_serialized: hidden ? hidden.serialized_leak === false : false,
    hidden_gpp_placeholder_absent: hidden ? hidden.gpp_placeholder_present === false : false,
  };
  state.hard_gate_result = Object.values(state.hard_gates).every(Boolean) ? 'PASS' : 'FAIL';
}

const provenStates = states.filter(state => state.qualification !== 'NOT_PROVEN' && state.hard_gate_result);
const failedStates = provenStates.filter(state => state.hard_gate_result === 'FAIL');
const requiredStateNames = ['current_assignee_active_step_get', 'authorized_non_assignee_get', 'ordinary_post', 'complete_approved_post_response', 'no_current_step_complete_get'];
...
const evidence = {
  ...
  objective: 'Prove GPP field-backed visibility is a subset of the real Gravity Flow Entry Detail native-visible fields in authentically reachable pinned-runtime states.',
  non_goals: ['production visibility repair', 'duplicate nativeDisplayStep test implementation', 'CSS visibility as oracle', 'label-based oracle'],
  runtime: {
    wordpress: '7.1.1',
  ...
  oracle: 'real Gravity Flow Entry Detail .entry-detail-view output; source IDs are inferred only from unique synthetic sentinel values written to known field IDs',
  mirrored_branch_scope: {
    pinned_gravity_flow: '3.1.0',
    current_assignee: 'covered',
    ordinary_non_post: 'covered',
  ...
  production_direction: disposition === 'CONFIRMED_DEFECT'
    ...
    : disposition === 'QUALIFIED_FOR_PINNED_RUNTIME'
      ? 'No production visibility repair is established for Gravity Flow 3.1.0; retain the qualification as an exact-version contract and re-qualify on host-version change.'
      : null,
};

fs.mkdirSync(artifactDir, { recursive: true });
const out = path.join(artifactDir, 'gpp-rp-wu02-entry-visibility-differential.json');
