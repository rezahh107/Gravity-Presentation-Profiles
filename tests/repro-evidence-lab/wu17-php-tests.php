...
wu17_test( 'WU17-PHP-001', 'one active shared Inbox visual profile is surface-owned', function () {
...
wu17_test( 'WU17-PHP-002', 'two form binding sets are independently active without visual identity', function () use ( $manifest ) {
...
wu17_test( 'WU17-PHP-003', 'production adapter adds one card column while preserving native row data columns', function () {
    ...
    wu17_assert( ! isset( $columns['gpp_school_filter'] ) && ! isset( $columns['gpp_due_filter'] ), 'Unsupported School/Due control was introduced.' );
    return array_keys( $columns );
} );

wu17_test( 'WU17-PHP-004', 'production adapter makes only the card column visible in native AG Grid config', function () {
    $config = array(
        'grids' => array(
            array(
                'grid_options' => array(
    ...
    $filtered = apply_filters( 'gravityflow_js_config_shared', $config );
    $options = $filtered['grids'][0]['grid_options'];
    wu17_assert( 'autoHeight' === $options['domLayout'], 'Native grid domLayout was not set to autoHeight.' );
    wu17_assert( true === $options['ensureDomOrder'], 'Native grid DOM order was not made deterministic.' );
    wu17_assert( true === $options['suppressHorizontalScroll'], 'Native horizontal scrolling was not suppressed.' );
    $visibility = array();
    foreach ( $options['columnDefs'] as $definition ) {
        $visibility[ $definition['field'] ] = empty( $definition['hide'] );
    }
    wu17_assert( false === $visibility['id'], 'Native id must stay in rowData but be visually hidden.' );
    wu17_assert( false === $visibility['date_created'], 'Native support columns must remain hidden.' );
    wu17_assert( true === $visibility[ InboxPresentationAdapter::CARD_COLUMN ], 'Card column must be the one visible column.' );
    return $visibility;
} );

wu17_test( 'WU17-PHP-005', 'multi-form cards share profile but resolve form-local name and national-id bindings', function () use ( $manifest ) {
    $cards = array();
...
wu17_test( 'WU17-PHP-006', 'NOT_PROVEN photo fails closed while PROVEN photo renders', function () use ( $manifest ) {
...
wu17_test( 'WU17-PHP-007', 'School and Due remain absent when bindings are not PROVEN', function () use ( $manifest ) {
...
wu17_test( 'WU17-PHP-008', 'GPP-owned entry date is Jalali/Persian without mutating host value', function () use ( $manifest ) {
...
wu17_test( 'WU17-PHP-009', 'overdue treatment is local and no whole-card overdue style exists', function () use ( $repo_root ) {
    ...
    return 'Only the due value owns overdue emphasis.';
} );

wu17_test( 'WU17-PHP-010', 'production adapter contains no independent query/search/paging/polling endpoint', function () use ( $repo_root ) {
    $source = file_get_contents( $repo_root . '/src/SRWF/GravityFlow/InboxPresentationAdapter.php' );
    foreach ( array( 'get_inbox_entries(', 'setQuickFilter(', 'applyTransaction(', 'register_rest_route(', 'wp_ajax_', 'setInterval(', 'fetch(' ) as $forbidden ) {
        wu17_assert( false === strpos( $source, $forbidden ), 'Presentation adapter introduced host-owned behavior: ' . $forbidden );
    }
    foreach ( array( 'gravityflow_columns_inbox_table', 'gravityflow_inbox_field_value', 'gravityflow_js_config_shared' ) as $seam ) {
        wu17_assert( false !== strpos( $source, $seam ), 'Expected source-backed native seam missing: ' . $seam );
    }
    return 'Native filters/config only; no parallel behavior path.';
} );

$result_path = trailingslashit( $artifact_dir ) . 'wu17-php-results.json';
file_put_contents( $result_path, wp_json_encode( array( 'suite' => 'WU17 production adapter PHP/runtime', 'results' => $GLOBALS['wu17_results'] ), JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE ) . "\n" );
