...
async function measure(name, viewport, constrained = false) {
  ...
  const state = await page.evaluate(({ safeProfile, tolerance }) => {
    ...
    if (!dossier || !host) throw new Error('Required Entry Detail nodes missing after admission.');
    const rect = node => {
    ...
    const descendants = [
      ...
      clipped: node.scrollWidth > node.clientWidth + tolerance,
    ...
    const focusables = Array.from(document.querySelectorAll('.gpp-entry-dossier a[href],.gpp-entry-dossier button:not([disabled]),.gpp-entry-print-utility button:not([disabled]),.gravityflow-status-box button:not([disabled]),.gravityflow-status-box textarea:not([disabled]),.gravityflow-timeline a[href]')).map(node => {
      ...
      return {
        ...
        inside_document: value.left >= -tolerance && value.right <= document.documentElement.clientWidth + tolerance,
      };
    });
    ...
    return {
      document: {
        ...
        horizontal_overflow: document.documentElement.scrollWidth > document.documentElement.clientWidth + tolerance,
        direction: getComputedStyle(document.documentElement).direction,
      },
      host: {
        selector: '#post-body-content',
        mode: host.dataset.gppWu10HostMode || 'FULL_WIDTH_HOST',
        rect: hostRect,
        content_box: hostContent,
        computed_width: hostStyle.width,
        max_width: hostStyle.maxWidth,
        overflowX: hostStyle.overflowX,
      },
      dossier: {
        count: document.querySelectorAll('.gpp-entry-dossier[data-gpp-entry-detail="ready"]').length,
        profile: dossier.dataset.gppProfileId || null,
        rect: dossierRect,
        clientWidth: dossier.clientWidth,
        scrollWidth: dossier.scrollWidth,
        computed_width: dossierStyle.width,
        max_width: dossierStyle.maxWidth,
        margin_inline_start: dossierStyle.marginInlineStart,
        margin_inline_end: dossierStyle.marginInlineEnd,
        overflowX: dossierStyle.overflowX,
        inside_host_content: dossierRect.left >= hostContent.left - tolerance && dossierRect.right <= hostContent.right + tolerance,
        internally_clipped: dossier.scrollWidth > dossier.clientWidth + tolerance,
      },
      descendants,
      longest_persian_fact: longest ? {
        text_length: (longest.textContent || '').trim().length,
        clientWidth: longest.clientWidth,
        scrollWidth: longest.scrollWidth,
        clipped: longest.scrollWidth > longest.clientWidth + tolerance,
      ...
      ownership: {
        ...
        timeline_ordered: noteTops.every((top, index) => index === 0 || top + tolerance >= noteTops[index - 1]),
      ...
      isolation: {
        full_width_stylesheet_loaded: styles.some(href => href.includes('srwf-gravity-flow-entry-detail-full-width.css')),
        post_body_display: getComputedStyle(document.querySelector('#post-body')).display,
      },
      focusables,
    };
  ...
  if (!state.focusables.length || state.focusables.some(item => !item.visible || !item.inside_document)) failures.push('FOCUSABLE_CONTROL_UNREACHABLE');
...
try {
  ...
  writeJson('wu10-harness-failure.json', { runtime, entry_url: entryUrl, completed_matrix: matrix, error: String(error?.stack || error) });
  await browser.close();
  throw error;
}
await browser.close();
...
const nativeHealthy = constrained.state.descendants.filter(item => ['status','timeline','print'].includes(item.id) && item.present).every(item => !item.clipped);
const attribution = constrained.hard_gate_failures.includes('DOSSIER_ESCAPES_HOST_CONTENT_BOX')
  ? (nativeHealthy ? 'GPP_INTRODUCED_CONTAINER_DEFECT' : 'GPP_AMPLIFIES_HOST_OR_NATIVE_DEFECT')
  : (matrix.every(item => item.passed) ? 'NO_REPRODUCIBLE_DEFECT' : 'NOT_PROVEN');
const output = {
  schema_version: '1.0.0',
  work_unit: 'GPP-RP-WU-10-ENTRY-CONSTRAINED-GEOMETRY',
  problem: 'P-14',
  surface: 'gravity_flow.entry_detail',
  profile: safeProfile,
  data_class: 'SYNTHETIC_NON_PII',
  runtime,
  host_contract: { authentic_host_selector: '#post-body-content', constrained_mode: '48rem browser-local constraint on the authentic host node; no dossier mock/reparent' },
  rtl_primary: 'server-rendered dossier dir=rtl with Persian synthetic values',
  ltr_smoke: matrix[0].state.document.direction === 'ltr' ? 'EXECUTED_HOST_LTR_SMOKE' : 'NOT_APPLICABLE',
  text_200_percent: 'NOT_PROVEN_RELIABLY_BY_EXISTING_HARNESS',
  attribution,
  matrix,
};
writeJson('wu10-entry-constrained-geometry.json', output);
console.log(JSON.stringify(output, null, 2));
const failures = matrix.flatMap(item => item.hard_gate_failures.map(failure => `${item.name}:${failure}`));
if (failures.length) {
  console.error(`WU10_GEOMETRY_FAIL ${failures.join(',')}`);
  process.exit(1);
}
