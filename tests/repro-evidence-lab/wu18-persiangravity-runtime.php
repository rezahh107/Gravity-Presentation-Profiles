...
$extract_entry_created = static function ( $html ) use ( $decode_text ) {
    $match = array();
    return 1 === preg_match( '~data-gpp-slot="entry\.created_at"[^>]*>.*?<dd>(.*?)</dd>~su', (string) $html, $match )
        ? $decode_text( $match[1] )
        : null;
};
...
$provider_for_utc = static function ( $raw ) {
    return PGR_Jalali_Presentation::format_datetime( new DateTimeImmutable( (string) $raw, new DateTimeZone( 'UTC' ) ) );
};
...
foreach ( $inbox_manifest['entry_records'] as $record ) {
    if ( is_array( $record ) && ! empty( $record['entry_id'] ) && ! empty( $record['form_id'] ) && (int) $record['form_id'] !== $mutated_entry_detail_form_id ) {
        $inbox_entry_id = (int) $record['entry_id'];
        break;
    }
}
...
try {
    ...
    try {
        ...
        wu18_assert( $decode_text( GFCommon::format_date( $out_of_range, false ) ) === $decode_text( PersianDateFormatter::formatDateTime( $out_of_range ) ), 'Provider null did not preserve native Gravity Forms presentation.' );
