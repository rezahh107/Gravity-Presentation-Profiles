<?php
/** Configure the deterministic Elementor Full Width host for Inbox-only visual diagnostics. */
...
$page_ids     = array_filter( array( $fixture['frontend_inbox_page_id'] ?? 0, $p06['authentic_block_page']['page_id'] ?? 0 ) );
...
if ( 'hello-elementor' !== get_option( 'stylesheet' ) || ! is_plugin_active( 'elementor/elementor.php' ) || ! is_plugin_active( 'elementor-pro/elementor-pro.php' ) ) {
	throw new RuntimeException( 'VISUAL_TEST_INFRASTRUCTURE_FAILURE: pinned Hello Elementor + Elementor + Elementor Pro host is not active.' );
}
...
foreach ( $page_ids as $page_id ) {
	...
	if ( 'elementor_header_footer' !== get_page_template_slug( $page_id ) ) {
		throw new RuntimeException( 'VISUAL_TEST_INFRASTRUCTURE_FAILURE: Elementor Full Width was not applied.' );
	}
...
$identity  = array(
	...
	'composition'                 => 'deterministic Elementor Full Width container/shortcode host fixture; Elementor provides only the outer host and no Inbox components are recreated by Elementor',
);
