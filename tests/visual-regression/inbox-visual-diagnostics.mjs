...
import { buildDesignEvidenceContext } from './evidence-context.mjs';
...
const emptyStateEvidencePath=path.join(out,'empty-state-seam.json');
if(!fs.existsSync(emptyStateEvidencePath)) throw new Error('VISUAL_TEST_INFRASTRUCTURE_FAILURE: empty-state seam qualification evidence is missing.');
const emptyStateQualification=JSON.parse(fs.readFileSync(emptyStateEvidencePath,'utf8'));
...
try {
  for (const scenario of contract.scenarios) {
    ...
    const evidenceContext=buildDesignEvidenceContext(scenario,emptyStateQualification);
    ...
    try {
      ...
      const designDelta=await runStage('design_convergence_compare',async()=>compareDesignFacts(designGeometry,geometry,contract.design_comparison_policy,scenario.design_relations,evidenceContext));
...
const designNotComparable=results.flatMap(result=>Object.entries(result.design_relation_results||{}).filter(([,evidence])=>evidence.classification==='NOT_COMPARABLE').map(([relation,evidence])=>({scenario:result.id,relation,...evidence})));
const nativeHostLimitations=results.flatMap(result=>Object.entries(result.design_relation_results||{}).filter(([,evidence])=>evidence.classification==='NATIVE_HOST_LIMITATION').map(([relation,evidence])=>({scenario:result.id,relation,...evidence})));
...
const evidenceLimitationScenarioCount=results.filter(result=>result.status==='EVIDENCE_LIMITATION').length;
const manifestStatus=failureCount?'VISUAL_CONTRACT_FAIL':warningCount?'VISUAL_REGRESSION_WARNING':evidenceLimitationScenarioCount?'EVIDENCE_LIMITATION':'PASS';
const manifest={mode:contract.mode,status:manifestStatus,repository_sha:repositorySha,baseline_identity:'NO_CROSS_COMMIT_RUNTIME_BASELINE_ADMITTED',scenarios:results.map(({geometry,dom,...r})=>r),warning_count:warningCount,design_relation_warning_count:designWarnings.length,design_relation_not_comparable_count:designNotComparable.length,native_host_limitation_relation_count:nativeHostLimitations.length,evidence_limitation_scenario_count:evidenceLimitationScenarioCount,failure_count:failureCount,infrastructure_failure_count:0,largest_visual_delta:largest?{scenario:largest.id,ratio:largest.screenshot_diff_ratio}:null,largest_geometry_delta:largestGeometry?{scenario:largestGeometry.scenario,relation:largestGeometry.relation,design:largestGeometry.design,runtime:largestGeometry.runtime,delta:largestGeometry.delta,tolerance:largestGeometry.tolerance}:null,first_meaningful_divergence:firstDesignWarning?{scenario:firstDesignWarning.scenario,relation:firstDesignWarning.relation,design:firstDesignWarning.design,runtime:firstDesignWarning.runtime,delta:firstDesignWarning.delta,tolerance:firstDesignWarning.tolerance,classification:firstDesignWarning.classification}:null,qualified_evidence_limitations:{not_comparable:designNotComparable.map(({scenario,relation,classification,reason,observations})=>({scenario,relation,classification,reason,observations})),native_host_limitations:nativeHostLimitations.map(({scenario,relation,classification,reason,evidence_ref})=>({scenario,relation,classification,reason,evidence_ref}))},first_divergence_note:firstDesignWarning?'First relation-level divergence outside the versioned design-comparison policy.':'No required comparable relation exceeded the versioned design-comparison policy.',evidence_ceiling:'Deterministic WU21 runtime observation; qualified native-host limitations and insufficient-cardinality relations are not product-regression proof; not production equivalence or Owner visual approval.'};
