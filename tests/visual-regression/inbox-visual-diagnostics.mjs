...
try {
  for (const scenario of contract.scenarios) {
    ...
    try {
      ...
      const hostIntegration=await runStage('host_integration',async()=>{const evidence=await page.evaluate((surfaceSelector)=>{const surface=document.querySelector(surfaceSelector);const body=document.body;const host=surface?.closest('.elementor-element[data-element_type="container"]');return {elementor_page_marker:body.classList.contains('elementor-page'),elementor_container_present:Boolean(host),surface_present:Boolean(surface),surface_dom_nested_in_elementor_container:Boolean(host&&surface&&host.contains(surface)),surface_horizontal_overflow:surface?Math.max(0,surface.scrollWidth-surface.clientWidth):null};},selectors.surface);writeJson(path.join(dir,'host-integration.json'),evidence);if(!evidence.elementor_container_present||!evidence.surface_present||!evidence.surface_dom_nested_in_elementor_container)throw new Error(`VISUAL_TEST_INFRASTRUCTURE_FAILURE: ${scenario.id} failed deterministic Elementor host integration.`);return evidence;});
