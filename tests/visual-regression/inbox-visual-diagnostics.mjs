...
try {
  for (const scenario of contract.scenarios) {
    ...
    try {
      ...
      const hostIntegration=await runStage('host_integration',()=>page.evaluate((surfaceSelector)=>{const surface=document.querySelector(surfaceSelector);const body=document.body;const r=surface?.getBoundingClientRect();const host=surface?.closest('.elementor-element[data-element_type="container"]');const hr=host?.getBoundingClientRect();return {elementor_canvas:body.classList.contains('elementor-template-canvas'),elementor_page:body.classList.contains('elementor-page'),elementor_container_present:Boolean(host),surface_present:Boolean(surface),surface_within_host:Boolean(r&&hr&&r.left>=hr.left-1&&r.right<=hr.right+1),document_horizontal_overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth};},selectors.surface));
      if(!hostIntegration.elementor_canvas||!hostIntegration.elementor_page||!hostIntegration.elementor_container_present||!hostIntegration.surface_present||!hostIntegration.surface_within_host||hostIntegration.document_horizontal_overflow>1) throw new Error(`VISUAL_TEST_INFRASTRUCTURE_FAILURE: ${scenario.id} failed neutral host integration.`);
