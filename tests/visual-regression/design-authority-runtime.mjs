...
export async function designFacts(page) {
  return page.evaluate(() => {
    ...
    const cards=[...document.querySelectorAll('#case-grid .case-card')].filter(visible);
    const surface=fact(document.querySelector('#inbox-view')), first=fact(cards[0]), second=fact(cards[1]), last=fact(cards.at(-1)), pager=fact(document.querySelector('#pagination'));
