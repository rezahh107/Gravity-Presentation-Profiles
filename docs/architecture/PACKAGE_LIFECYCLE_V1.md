...
Each class state is replaced as one option value. Lifecycle code computes the complete next state before `StateStore::commit()`. The WordPress store serializes class-specific writers in two layers: a deterministic connection-scoped database advisory lock provides inter-session serialization, and a private request-local same-store guard prevents recursive entry on the same runtime database context. The advisory lock is derived from the current database/site/store identity, acquired non-blockingly through the existing `$wpdb` connection, held across revision read, `update_option()` and `get_option()` readback, and released in `finally`; database-session termination also recovers an orphaned lock. The request-local guard is derived only from already-available `$wpdb` object/site/store identity, becomes active after successful `GET_LOCK()` acquisition and before the protected state read, rejects nested commits for that same logical store before a second advisory-lock acquisition or lifecycle write, and is cleared when the outer critical section exits. Visual and binding lifecycle options therefore remain independently guarded. No persistent coordination option or parallel state authority is created. A failed commit leaves the prior state authoritative. The stores are intentionally separate so a visual operation cannot partially commit binding state, or vice versa.
