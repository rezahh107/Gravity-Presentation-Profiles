...
Each class state is replaced as one option value. Lifecycle code computes the complete next state before `StateStore::commit()`. The WordPress store serializes class-specific writers in two layers: a deterministic connection-scoped database advisory lock provides inter-session serialization, and a private request-local same-store guard prevents recursive entry on the same runtime database context. The request-local guard is derived only from already-available `$wpdb` object/site/store identity and is activated before advisory-lock-name derivation or any hookable database query. It remains active across non-blocking `GET_LOCK()` acquisition, revision read, `update_option()` state write, `get_option()` readback, and the `RELEASE_LOCK()` attempt, then is cleared in bounded `finally` cleanup on every outer exit path. A nested commit for that same logical store therefore fails before issuing any lifecycle database query or state write. The advisory lock remains derived from the current database/site/store identity, is acquired through the existing `$wpdb` connection, and provides cross-session exclusion; database-session termination recovers an orphaned lock. Visual and binding lifecycle options retain independent guard and advisory-lock identities. No persistent coordination option or parallel state authority is created. A failed commit leaves the prior state authoritative. The stores are intentionally separate so a visual operation cannot partially commit binding state, or vice versa.
