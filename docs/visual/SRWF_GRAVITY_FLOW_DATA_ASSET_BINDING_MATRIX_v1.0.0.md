...
| IN-06 | Inbox state | Due / overdue | Gravity Flow Due only if authoritative/material | No authoritative SRWF Due source/config is admitted; target runtime configuration is unbound | NOT_PROVEN | EV-GPP-BASE, EV-SRWF-SFC, EV-SRWF-MAP | Omit Due state/filter/sort because no authoritative material Due is available; do not claim target runtime absence. |
...
| PF-13 | Gender option group: `female`, `male` | Gravity Forms/SRWF candidate | SRWF `STUDENT_GENDER` has semantic values `0`=دختر/زن and `1`=پسر/مرد; runtime field ID unbound | Explicit semantic mapping: authoritative value `0` -> `female`; `1` -> `male`; missing/unbound value -> neither. No default selection. Production execution still requires the target field binding. | EV-HTML, EV-SRWF-SFC | UNBOUND | Both boxes remain blank until exact target field + explicit print option mapping are bound. |
...
| PF-16 | Graduate status: `graduated` | Gravity Forms/SRWF candidate | SRWF `GRADUATION_STATUS` has `0`=فارغ‌التحصیل and `1`=دانش‌آموز; runtime field ID unbound | Explicit semantic mapping: authoritative value `0` -> `graduated` checked; `1` -> unchecked; missing/unbound value -> blank/manual. No default. Production execution still requires the target field binding. | EV-HTML, EV-SRWF-SFC | UNBOUND | Checkbox remains blank until exact binding/mapping. |
| PF-17 | Registration location: `loc_central`, `loc_golestan`, `loc_sadra` | Gravity Forms/SRWF | SRWF `REGISTRATION_CENTER` has authoritative values `0`=مرکز, `1`=گلستان, `2`=صدرا; runtime field ID is unbound | Explicit semantic mapping for this WU: authoritative `REGISTRATION_CENTER` value `0` -> `loc_central`, `1` -> `loc_golestan`, `2` -> `loc_sadra`; missing/unbound value -> none. The mapping does not authorize defaulting to `0`. | EV-HTML, EV-SRWF-SFC | UNBOUND | All location options remain blank/manual until the target field binding is proven; an absent value never defaults to central. |
