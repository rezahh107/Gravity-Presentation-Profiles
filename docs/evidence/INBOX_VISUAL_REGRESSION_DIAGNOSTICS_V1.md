...
The integrated stage records immutable host package hashes/commit, Gravity versions, theme, browser, repository SHA, capture scope, and design-authority hash. Missing or mismatched dependency identities, a wrong Full Width template, an active `SRWF-Host-Companion`, or failure to render the Inbox DOM inside the real Elementor container is an infrastructure failure. Elementor/theme shell geometry and document-level overflow are not authenticity gates; overflow diagnostics are scoped to the Inbox surface itself. No complete Elementor site export is required or validated by this Inbox-only contract. PersianGravity, Vazir/Vazirmatn, and GTB are not fabricated in this fixture: no repository evidence admits an active Inbox responsibility for those optional companions in WU21. Their absence is therefore recorded rather than falsely presented as integrated behavior.
