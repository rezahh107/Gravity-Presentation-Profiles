...
The forward host is pinned Hello Elementor plus pinned Elementor. Hello supplies the minimal theme foundation; Elementor supplies its supported neutral `elementor_canvas` Page template and a deterministic full-width container with one native shortcode widget. That test-only widget delegates to the existing synthetic Page content, including `do_blocks()` for the authentic Block fixture; it does not reproduce Gravity Flow behavior. The fixture contains no Elementor-built Inbox cards, heading, search, summary, grid, or pager. Runtime evidence proves that Elementor recognizes and renders the Page, verifies the theme/plugin identities and template assignment, and checks the bounded GPP surface and page-level overflow before capture.

The historical WU21 functional lane remains pinned to WordPress 6.8.3 and PHP 8.2.34 and executes before host activation. A subsequent, explicitly named integrated visual-host stage installs Elementor 3.35.9 and Hello Elementor 3.5.1 from byte-verified pinned packages, activates the neutral host, then runs diagnostics. Thus historical functional observations are not relabeled as proof of the visual host. `SRWF-Host-Companion` is intentionally absent and is asserted inactive: it is not a forward-runtime dependency.

The integrated stage records immutable host package hashes/commit, Gravity versions, theme, browser, repository SHA, and design-authority hash. Missing identities or an Elementor Canvas assignment mismatch is an infrastructure failure, never a fallback to the minimal WU21 screenshot path. PersianGravity, Vazir/Vazirmatn, and GTB are not fabricated in this fixture: no repository evidence admits an active Inbox responsibility for those optional companions in WU21. Their absence is therefore recorded rather than falsely presented as integrated behavior.
