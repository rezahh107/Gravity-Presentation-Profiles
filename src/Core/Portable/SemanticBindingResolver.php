...
final class SemanticBindingResolver {
    ...
    private function contextMatches( $binding_context, $runtime_context ) {
        if ( $binding_context['installation_source_ref']['installation_id'] !== $runtime_context['installation_id'] ) {
            return false;
        }

        if ( $binding_context['form_source_ref']['form_id'] !== $runtime_context['form_id'] ) {
            return false;
        }
        ...
        return null === $binding_context['entry_source_ref'] || $binding_context['entry_source_ref']['entry_id'] === $runtime_context['entry_id'];
    }
    ...
    private function selectMostSpecific( $candidates, $context ) {
        ...
        foreach ( $candidates as $candidate ) {
            $entry_ref = $candidate['context']['entry_source_ref'];
            if ( null !== $entry_ref && $entry_ref['entry_id'] === $context['entry_id'] ) {
                $exact[] = $candidate;
            }
        }
